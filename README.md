...
依据《XToolsCRM OPEN API》文档（2026-05-22 导出，68 个接口）整理的接入准备材料与 Python 客户端。签名算法已用文档中 29 组示例逐一核验。
...
│   ├── errors.py             异常定义
│   ├── catalog.json          68 个接口的机器可读清单（cmd / dt / act / 模块）
│   └── api/                  按模块封装：customers / orders / products / finance
├── scripts/
...
└── docs/
    ...
    ├── 03_接口清单.md              68 个接口按模块分组
    ├── 04_首批四模块字段映射.md    客户 / 订单 / 产品 / 财务的字段与枚举
    ├── 05_同步方案设计.md          架构、读取策略、写回流程、阶段计划
    ├── XToolsCRM_OPEN_API_参考_20260522.md   原文档整理版（逐接口）
